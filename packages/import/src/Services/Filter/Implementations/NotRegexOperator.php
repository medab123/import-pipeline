...
final class NotRegexOperator extends AbstractFilterOperator
{
    ...
    {
        return 'Not Regex Match';
    }

    public function getDescription(): string
    {
        return 'Check if the value does not match the regular expression pattern';
    }
    ...
    protected function doApply(mixed $dataValue, mixed $filterValue, array $options): bool
    {
        ...
        if (! $this->isStringValue($filterValue)) {
            return true; // Not regex is true for non-string filter values
        }
        ...
        try {
            ...
            return $result !== 1; // Return true if no match (not regex)
        } catch (\Exception $e) {
            // Invalid regex pattern - consider it as not matching
            return true;
        }
    }
