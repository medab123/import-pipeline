...
const form = useForm({
  dealer_id: String(props.scrap.dealerId),
  ftp_file_path: props.scrap.ftpFilePath,
  provider: props.scrap.provider,
})

const submit = () => {
  form.put(route('dashboard.scraps.update', props.scrap.id))
}
</script>

<template>
  <Head title="Edit Scrap Source" />

  <Default>
    <PageHeader
      title="Edit Scrap Source"
      :description="`Update scrap source from ${scrap.provider}`"
    >
      <template #actions>
      ...
      <form @submit.prevent="submit">
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Update the FTP scrap source configuration.
            </CardDescription>
          </CardHeader>
          ...
          <CardFooter class="flex justify-end gap-2 border-t pt-4">
            ...
            <Button type="submit" :disabled="form.processing">
              {{ form.processing ? 'Updating...' : 'Update Scrap Source' }}
            </Button>
          </CardFooter>
        </Card>
      </form>
